...
const PublicServiceForm = () => {
  ...
  const validateLink = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:4000'}/share-service/${linkId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:4000'}/share-service/${linkId}/submit`, {
        method: 'POST',
        headers: {
