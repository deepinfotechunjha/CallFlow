                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredOrders.map(order => (
                    <React.Fragment key={order.id}>
                      <tr onClick={() => openModal('detail', order)} className={`hover:bg-gray-50 transition-colors cursor-pointer ${order.status === 'CANCELLED' ? 'opacity-60' : ''}`}>
                        ...
                        <td className="px-4 py-3">
                          ...
                          {order.holds?.length > 0 && (
                            <button onClick={e => { e.stopPropagation(); toggleHolds(order.id); }} className="ml-2 text-xs text-yellow-600 hover:underline">
        ...
        {loading ? (
          ...
          filteredOrders.map(order => (
            ...
            >
              ...
              {order.holds?.length > 0 && (
                <div className="mb-3">
                  <button onClick={e => { e.stopPropagation(); toggleHolds(order.id); }} className="text-xs text-yellow-600 font-medium hover:underline">
